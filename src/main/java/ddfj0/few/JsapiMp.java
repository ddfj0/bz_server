...
public class JsapiMp {
    ...
    public void fnStartService() {
        try {
            ...
            System.out.println("fnStartService() : jsapi 启动微信支付服务成功！ ");
        } catch (Exception ee) {
            System.out.println("fnStartService() error: " + ee.getMessage());
        }
    }
    
    public TreeMap<String, String> fnWePayPrePayMp(String appid, String openid, String outTradeNo, String description, Integer iAmount) {
        ...
        try {
            ...
            WechatPaySign sign = sign( appid, response.getPrepayId());
...
class WechatOpenId {
    ...
    public Date onlinDate;
    public WechatOpenId(String openid, Date onlinDate) {
        this.openid = openid;
        this.onlinDate = onlinDate;
    }

    public boolean fnMoveExpired() {
        Date now = new Date();
        if( now.getTime() - onlinDate.getTime() > 1000 * 60 * 8 ) {  /// fail 8 minutes， alive 5 minutes
            return true;
        }
        return false;
    }

    public static boolean fnOpenIdValied(String openid) {
        //Date now = new Date();
